...
-   `01_data_cleaning.R`\
    Cleans and standardises the raw CLHC dataset, maps free-text to predefined categories, applies the positive-FibroScan restriction, produces descriptive statistics, and saves the cleaned datasets.
...
Package versions are recorded in `renv.lock`. After opening the R project, restore the package environment by running the following terminal commands one at a time:
