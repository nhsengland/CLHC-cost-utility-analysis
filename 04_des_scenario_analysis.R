## 4) CLHC cirrhosis-management scenario analysis discrete event simulation (DES) script ##

# This is the fourth script in the CLHC Cost-Utility Analysis repository.
# It runs the DES scenario analysis, modelling the impact of an author-defined,
# assumed 10% reduction in the decompensation hazard following cirrhosis identification.

# It is identical to the base-case script except for the post-identification cirrhosis-management 
# decompensation hazard reduction ("decompensation_rate_ratio_after_identification"), and the 
# cirrhosis management costing process, which only applies the cirrhosis management cost
# ("annual_compensated_cirrhosis") following assumed cirrhosis identification (i.e. from model entry 
# for the CLHC arm and from surveillancereferral or symptomatic HCC diagnosis in the usual care arm).
...
des_output_directory <- file.path(clhc_project_directory, "outputs", "DES", "cirrhosis_management_scenario")
...
params <- list(
  ...
  natural_history = list(
    ...
    # Exploratory scenario assumption: a 10% reduction in the decompensation
    # hazard after cirrhosis identification
    decompensation_rate_ratio_after_identification = 0.90,
...
get_ongoing_cost_annual <- function(
    cirrhosis_state,
    cirrhosis_identified,
    treatment,
    time_point,
  ...
  # Decompensated-cirrhosis costs accrue regardless of prior identification,
  # reflecting healthcare use following decompensation
  if (cirrhosis_state == "decompensated") {
    
    annual_cost <-
      annual_cost +
      params$costs$annual_decompensated_cirrhosis
    
    # Compensated-cirrhosis management costs accrue only after identification
  } else if (
    cirrhosis_state == "compensated" &&
    isTRUE(cirrhosis_identified)
  ) {
    
    annual_cost <-
      annual_cost +
      params$costs$annual_compensated_cirrhosis
  }
  ...
  if (
    ...
    if (treatment == "resection_ablation") {
      ...
      annual_cost <-
        annual_cost +
        params$costs$annual_post_resection_ablation
    }
    ...
    if (treatment == "systemic") {
      ...
      annual_cost <-
        annual_cost +
        params$costs$annual_systemic_therapy
    }
  }
...
accrue_between_events <- function(elapsed, time_start, cirrhosis_state, cirrhosis_identified, hcc_state, treatment, treatment_start_time, params) {
  ...
  annual_cost <- get_ongoing_cost_annual(
    ...
    cirrhosis_identified = cirrhosis_identified,
    treatment = treatment,
...
# ================================================================================================
# 4a. Pre-generate paired baseline cirrhosis severity & background mortality
# Sampled once before running both arms, so each individual has the same baseline severity and background mortality
# ================================================================================================
...
schedule_events <- function(
    current_time, patient, arm, cirrhosis_state, cirrhosis_identified, hcc_state, 
    hcc_diagnosed, treatment, treatment_start_time, under_surveillance, 
    next_surveillance_time, background_death_time, usual_care_ever_referred, params
) {
  ...
  # Decompensated cirrhosis onset
  if (cirrhosis_state == "compensated") {
    
    decompensation_rate <-
      params$natural_history$decompensation_incidence_annual
    
    if (isTRUE(cirrhosis_identified)) {
      decompensation_rate <-
        decompensation_rate *
        params$natural_history$
        decompensation_rate_ratio_after_identification
    }
    
    event_times["decompensation"] <-
      current_time +
      sample_time_to_event(decompensation_rate)
  }
...
simulate_one_patient <- function(patient, arm, params, keep_event_log = FALSE) {
  ...
  if (arm == "CLHC") {
    cirrhosis_identified <- cirrhosis_state == "compensated"
  ...
  while (alive && current_time < max_time) {
    ...
    # Disease onset and progression events  
      ...
      cirrhosis_state <- "decompensated"
      cirrhosis_identified <- TRUE
      ever_decompensated <- TRUE
    ...
    # HCC diagnosis events
      ...
      if (hcc_state != "none" && !hcc_diagnosed) {
        # Symptomatic HCC diagnosis also identifies the underlying cirrhosis
        cirrhosis_identified <- TRUE
  ...
  patient_result <- tibble(
    ...
    ever_decompensated = ever_decompensated,
...
summarise_simulation_outputs <- function(
  ...
  summary_by_arm <- all_results %>%
    ...
    summarise(
      ...
      n_decompensated = sum(ever_decompensated),
      decompensation_rate = mean(ever_decompensated),
